...
# `externalBin` entry exists on disk for the current target triple on
# EVERY `cargo build`/`clippy`/`test`, not just `tauri build` (see
# .superpowers/sdd/2026-09-01-packaging/task-1-report.md). None of those
# commands execute the sidecars for real — the tests that do are
# #[ignore]d — so empty placeholders satisfy the check without the real
# mpv source build or yt-dlp/Deno downloads that release.yml does.
#
